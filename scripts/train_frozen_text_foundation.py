"""BRIDGING EXPERIMENT Step 1: trains a linear-head text classifier on the
FROZEN, non-contextual, isolated-utterance text cache
(masked_mean_second_to_last_layer pooling, data/meld/cache/text/{split}/...
-- cache_version 4, the pre-Phase-T features) so the frozen end of the
subsumption/bridging claim (docs/PHASE_N5A.md) can be measured under the
SAME residual methodology as the fine-tuned end, instead of the old
concat-linear-probe comparison (docs/DIAGNOSIS.md's +0.0160 gap).

Locked optimizer settings match Phase N4's RECIPE.md-locked GNN values
(scripts/train_rapport.py), so this frozen-era foundation is trained under
the identical optimizer/schedule/selection regime as every other anchor in
this study -- only the model (linear vs. LoRA-RoBERTa vs. GNN) and the
input features (frozen "text" cache vs. text_ctx) differ: AdamW lr=1e-4,
CosineAnnealingLR(T_max=100), dropout=0.5, early-stop patience 10 on val
MACRO F1, batch=16 dialogues, grad_clip=1.0, bf16 autocast on CUDA, plain
CE loss, seed 42.

Also caches this checkpoint's own 7-d logits for every split to
data/meld/cache/text_logits/{split}/dia{d}_utt{u}.pt (paired with the
existing "text" feature cache per MELDCachedDataset's
f"{text_cache_subdir}_logits" convention), so `--text_cache_subdir text
--residual` on scripts/train_rapport.py can consume it directly -- the
same role build_text_ctx_cache.py plays for the Phase T contextual
encoder, but reading straight from the already-cached frozen text_feat
(no re-encoding needed; the backbone was never fine-tuned in this cache).

Usage:
    uv run python -m scripts.train_frozen_text_foundation
"""
...
# RECIPE.md-locked GNN values, reused verbatim (docs/PHASE_N4.md /
# scripts/train_rapport.py).
LR = 1e-4
DROPOUT = 0.5
MAX_EPOCHS = 100
...
NUM_WORKERS = 4
GRAD_CLIP = 1.0
FROZEN_POSTHOC_TAU = 0.25  # reported only, never used for selection (matches every other anchor in this study)

SEED = 42
TEXT_CACHE_SUBDIR = "text"  # the frozen, non-contextual, pre-Phase-T cache (cache_version 4)
LOGITS_OUT_SUBDIR = f"{TEXT_CACHE_SUBDIR}_logits"
RUN_NAME = f"frozen_text_foundation_seed{SEED}"
...
def build_dataloaders(batch_size: int = BATCH_SIZE, num_workers: int = NUM_WORKERS) -> dict[str, DataLoader]:
    ...
    for split in ("train", "dev", "test"):
        dataset = MELDCachedDataset(processed_dir / f"{split}.parquet", cache_dir, text_cache_subdir=TEXT_CACHE_SUBDIR)
...
def train(seed: int, run_dir: Path) -> tuple[dict, FrozenTextLinearProbe]:
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        # Selection/early-stopping metric: val MACRO F1 (matches every other
        # anchor's selection metric in this study, Phase N4's exception to
        # RECIPE.md's original weighted-F1 convention).
...
def cache_logits(model: FrozenTextLinearProbe, device: torch.device) -> dict[str, int]:
    """Writes this checkpoint's own 7-d classifier logits (z_text) for every
    utterance, all splits, to data/meld/cache/text_logits/{split}/... -- the
    same role build_text_ctx_cache.py plays for the Phase T contextual
    encoder, but reading straight from the already-cached frozen text_feat
    (no re-encoding needed; the backbone was never fine-tuned)."""
    model.eval()
    processed_dir = PROJECT_ROOT / "data" / "meld" / "processed"
    cache_dir = PROJECT_ROOT / "data" / "meld" / "cache"
    ...
    manifest = {
        "cache_version": "text_frozen_logits_v1",
