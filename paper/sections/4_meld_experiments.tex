...
% MELD dataset citation (Poria et al., 2019) has NO entry in
% docs/CITATIONS.md -- left as an undefined TODO key, flagged in the
% final STOP report rather than invented.
...
We then replace only the foundation. The LoRA-fine-tuned contextual text encoder of Section~\ref{sec:meld-foundation} (test weighted F1 $0.6403 \pm 0.0045$ --- itself exceeding the published full-system performance of DialogueRNN, MMGCN, EmoShiftNet~\cite{dialoguernn2019,mmgcn2019,emoshiftnet2025}, and our own prior SocialArcNet (published: $0.62$ weighted F1)~\cite{socialarcnet2026}, on this benchmark) supplies the anchor logits and embeddings; the architectural stack, the acoustic and visual features, the residual construction, the recipe, and the seeds are unchanged. Two experimental designs bracket the question.
...
The contrast between Sections~\ref{sec:meld-frozen} and~\ref{sec:meld-finetuned} is the boundary claim, measured under uniform methodology: the same components, the same attribution instrument, the same recipe, the same seeds --- a $+0.036$ contribution on frozen features and a null-to-negative contribution on fine-tuned ones. What changed is the foundation, and on the evidence of Section~\ref{sec:k-sweep}, what matters about the foundation is the fine-tuning itself rather than its contextual input window.
...
\caption{MELD mechanism: scratch retraining vs.\ residual attribution. The baseline$\to$full-stack gap shrinks $63\%$ once the incidental-presence confound is removed by construction.}
\label{fig:mechanism}
\end{figure*}
